...
const MovieDetails = () => {
  ...
  const getShow = async () => {
    try {
      ...
      if (data.success){
        ...
        toast.error("Error fetching show with id:", id, data.show)
      }
    } catch (error) {
      console.log(error)
    }
  ...
  const handleFavorite = async () => {
    try {
      if (!user) return toast.error("Please login to save movies as favorites!")
        ...
        <div className='flex items-center gap-4 w-max px-4'>
          {show.movie.casts.slice(0,12).map((cast, index) => (
            <div key={index} className='flex flex-col items-center text-center'>
              ...
              <p className='font-medium text-smmt-3'>{cast.name}</p>
            </div>
          ))}
        </div>
      </div>
