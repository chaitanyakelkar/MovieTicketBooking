  ...
  const getAllBookings = async () => {
    try {
      ...
      if (data.success){
        ...
        toast.error("Error fetching all bookings", data.message)
      }
    } catch (error) {
      console.log(error)
    }
        ...
        <table className='w-full border-collapse rounded-md overflow-hidden text-nowrap'>
          <thead>
            <tr className='bg-primary/20 text-left text-white'>
              ...
              <th className='p2 font-medium'>Amout</th>
            </tr>
          </thead>
          <tbody className='text-sm font-light'>
            {bookings.map((item, index) => (
              <tr key={index} className='border-b border-primary/10 bg-primary/5 even:bg-primary/10'>
                ...
                <td className='p-2'>{item.bookedSeats.join(", ")}</td> {/* Object.keys(item.bookedSeats).map(seat => item.bookedSeats[seat]).join(", ") <-previously now improved */}
